...
function RecentlyTest() {
    ...
    async function getUserData() {
      try {
        ...
        const { data } = await axios.get(
          `http://192.168.100.2:5000/api/users/ret/${user._id}`,
          config
        );
  ...
  const UpdateLastSeenAndStatus = async (status) => {
    try {
      ...
      const { data } = await axios.put(
        "http://192.168.100.2:5000/api/users/stat",
        {
          userId: user._id,
          status: status,
  ...
  return (
    <View>
      ...
      <Text>
        the time before user moves out of the app is at {lastseendateandtime}{" "}
      </Text>
