...
const Saved = () => {
  ...
  const addToWislistTraveler = async (_id) => {
    ...
    try {
      if (selectedTab === 1) {
        const travelerDataArr = travelerData.filter((item) => _id !== item._id);
        await AsyncStorage.setItem(
          "@savedTravelers",
          JSON.stringify(travelerDataArr)
        );
      } else if (selectedTab === 2) {
        const buyerDataArr = buyerData.filter((item) => _id !== item._id);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        ) : (selectedTab === 1 && travelerData.length === 0) ||
          (selectedTab === 2 && buyerData.length === 0) ? (
          <View
            style={{
              alignItems: "center",
              paddingTop: 60,
              ...
              <View>
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <TravelerCard
                      item={item}
                      addToWislistTraveler={addToWislistTraveler}
                    />
                  )}
