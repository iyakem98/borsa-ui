...
const ConnectScreen = () => {
  ...
  const { fetchAgain, setfetchAgain, chatSelected, setchatSelected } =
    ChatState();
  ...
  const filterBuyers = async () => {
    if (!buyerD && !buyerS) {
      ...
      let { data } = await axios.get(
        `http://143.198.168.244/api/buyers`,
        config
      );
  ...
  const getUsers = async () => {
    ...
    if (Math.ceil(totTraveler / 10) >= pageTraveler) {
      await axios
        .get(
          `http://143.198.168.244/api/travels?page=${pageTraveler}&limit=10`,
          config
        )
        .then((data, total) => {
    ...
    if (Math.ceil(totBuyer / 10) >= pageBuyer) {
      await axios
        .get(
          `http://143.198.168.244/api/buyers?page=${pageBuyer}&limit=10`,
          config
        )
        .then((data, total) => {
          setB(data.data.data);
              ...
              {isBuyer ? (
                  ...
                  {filterBuyer == 0 &&
                    (buyersStatus == "All" ? (
                      ...
                      <>
                        ...
                        >
                          Showing results for {buyerS ? buyerS : "-"} to{" "}
                          {buyerD ? buyerD : "-"}
                        </Text>
                        <TouchableOpacity
                          onPress={() => {
                ...
                >
                  {filterTraveler == 0 &&
                    (travelersStatus == "All" ? (
                      ...
                      <>
                        ...
                        >
                          Showing results for {travelerS ? travelerS : "-"} to{" "}
                          {travelerD ? travelerD : "-"}
                        </Text>
                        <TouchableOpacity
                          onPress={() => {
                  ...
                  <FlatList
                    ...
                    refreshControl={
                      <RefreshControl
                        refreshing={travelerRefreshing}
                        onRefresh={onTravelerRefresh}
                      />
                    }
                  />
                  {loadingTraveler && (
