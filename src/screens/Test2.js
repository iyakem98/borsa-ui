  ...
  const saveImg = async () => {
    ...
    try {
      ...
      const data = await axios.post(
        "http://192.168.100.2:5000/api/images/addImg",
        formData,
        config
      );
      //   headers: {
  ...
  const getImgs = async () => {
    ...
    try {
      const { data } = await axios.get(
        "http://192.168.100.2:5000/api/images/retrieveImg"
      );
      setgetPics(data);
      // console.log(data)
    } catch (err) {
          ...
          getPics.map((img) => {
            ...
            return (
              <Image
                ...
                style={styles.img}
                source={{
                  uri: `http://192.168.100.2:5000/images/${img.image}`,
                }}
              />
            );
          })}
